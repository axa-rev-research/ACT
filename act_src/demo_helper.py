...
Usage:
    from demo_helper import load_dataset, train_model, evaluate_model, plot_tree
...
# Add local textgrad and act_src to path
_project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
_act_src_path = os.path.join(_project_root, 'act_src')
_textgrad_path = os.path.join(_project_root, 'textgrad')

for _path in [_act_src_path, _textgrad_path]:
    if os.path.exists(_path) and _path not in sys.path:
        sys.path.insert(0, _path)

if os.path.exists(_textgrad_path):
    print(f"[INFO] Using local TextGrad from: {_textgrad_path}")
else:
    print("[WARNING] Local TextGrad not found, using installed version")

if os.path.exists(_act_src_path):
    print(f"[INFO] Using ACT source from: {_act_src_path}")
else:
    raise FileNotFoundError(f"ACT source not found at: {_act_src_path}")
...
def load_dataset(task_name="DIAGNO3", llm_engine=None):
    ...
    Args:
        task_name: Name of the task to load (default: "DIAGNO3")
...
def train_model(
    ...
    min_gini=0.05,
    stop_min_samples=10,
    optimization_constraints="exploring",
    ...
    Args:
        ...
        min_gini: Gini stopping threshold (default: 0.05)
        stop_min_samples: Min samples to continue splitting (default: 10)
        optimization_constraints: Constraint mode: "basic", "exploring", or "exploiting" (default: "exploring")
        two_stage_training: Enable exploring -> exploiting stage switch (default: False)
    ...
    model.fit(
        ...
        start_opt_constraints=optimization_constraints,
...
def evaluate_model(
    ...
    accuracy_list, nbr_fallbacks, nbr_refused = fn.eval_dataset(
        test_set=dataset,
        eval_fn=eval_fn,
        model=model,
        keyword=keyword,
    ...
    if verbose:
        ...
        if nbr_fallbacks > 0:
            print(f"[INFO] Fallbacks: {nbr_fallbacks}")
        if nbr_refused > 0:
            print(f"[INFO] Refused: {nbr_refused}")
...
def print_quick_start():
    ...
    2. Load Dataset:
       train, val, test, eval_fn = load_dataset("DIAGNO3", llm_engine)
